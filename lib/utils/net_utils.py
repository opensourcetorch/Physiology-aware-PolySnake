...
def load_model(net, optim, scheduler, recorder, model_dir,pretrain_dir, resume=True, epoch=-1):
    import ipdb


    print('load model {}'.format(model_dir))
    print('load pretrain model {}'.format(pretrain_dir))
    if not resume:
        os.system('rm -rf {}'.format(model_dir))
        return 0
    if os.path.exists(pretrain_dir):
        pths = [int(pth.split('.')[0]) for pth in os.listdir(pretrain_dir)]
        if len(pths) == 0:
        ...
        if epoch == -1:
            ...
            pth = epoch
        print('load model: {}'.format(os.path.join(pretrain_dir, '{}.pth'.format(pth))))
        pretrained_model = torch.load(os.path.join(pretrain_dir, '{}.pth'.format(pth)))
        model_dict =net.state_dict()# pretrain classifier
        pretrained_dict = {k: v for k, v in pretrained_model['net'].items() if k in model_dict}# pretrain classifier
        # print("pretrained_dict",pretrained_dict.keys())
        model_dict.update(pretrained_dict)# pretrain classifier
        net.load_state_dict(model_dict)# pretrain classifier
        # ipdb.set_trace()
