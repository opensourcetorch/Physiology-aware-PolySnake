...
def snake_collator(batch):
    ...
    batch_tmp = batch
    for i in range(len(batch_tmp)):
        if batch[i] == None:
            for j in range(len(batch_tmp)):
                if batch[j] != None:
                    batch[i] = batch[j]
                    break
    batch = batch_tmp

    ret = {'inp': default_collate([b['inp'] for b in batch]),'inp_c': default_collate([b['inp_c'] for b in batch]), 'mask': default_collate([b['mask'] for b in batch]),
           'plaque_patch': default_collate([b['plaque_patch'] for b in batch]),
           'plaque_patch_2d': default_collate([b['plaque_patch_2d'] for b in batch]),
           'mask_seg': default_collate([b['mask_seg'] for b in batch]),
           'mask_seg_volume': default_collate([b['mask_seg_volume'] for b in batch]),
           'path': default_collate([b['path'] for b in batch]),
           'mask_dis': default_collate([b['mask_dis'] for b in batch]),
    ...
    ct_img_idx = torch.zeros([batch_size, ct_num], dtype=torch.int64)
