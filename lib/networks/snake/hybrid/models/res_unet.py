...
class UpConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,kernel_size=2):
        super().__init__()
        self.transconv = nn.ConvTranspose3d(in_channels, out_channels, kernel_size,
                                            stride=2, padding=0)
...
class ResUNet_cls(nn.Module):
    ...
    def __init__(self, in_channels=1, out_channels=1, down_blocks=[32, 64, 128, 256, 512],
        ...
        for b_inx, up_block in enumerate(self.up_blocks):
            ...
            if output_channel == 32:
                # print("32done")
                self.TransUpBlocks.append(UpConv(input_channel, output_channel, kernel_size=[1, 2, 2]))
                self.BlocksUp.append(ResBlock(input_channel, output_channel, stride=1, p=p))
                continue
            self.TransUpBlocks.append(UpConv(input_channel, output_channel))
            self.BlocksUp.append(ResBlock(input_channel, output_channel, stride=1, p=p))
        ...
        # final convolution layer
        self.fl = nn.Conv3d(self.up_blocks[-1], out_channels[0], kernel_size=1)
        self.layer1 = nn.Sequential(
            nn.Conv2d(15, 15, kernel_size=5, stride=1, padding=0),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2))
        self.layer2 = nn.Sequential(
            nn.Conv2d(15, 15, kernel_size=2, stride=1, padding=0),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2))
        self.fl_1 = nn.Sequential(nn.Linear(22 * 22, out_channels[1]),
...
def ResUNet_cls3d(in_channels=1, out_channels=[1,10,4], p=0.0):
    return ResUNet_cls(in_channels=in_channels, out_channels=out_channels, down_blocks=[32, 64, 128],
                       up_blocks = [128, 64, 32], bottleneck = 256, p=p)
